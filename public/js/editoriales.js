(function() {
    ...
    function cargar() {
        ...
        if (esBib) {
            document.getElementById('btnNuevo').innerHTML = '<button class="btn btn-primary" id="btnNuevoEd">+ Nueva</button>';
            document.getElementById('btnNuevoEd').addEventListener('click', function() { formulario(); });
        }
        api('/editoriales').then(d => {
            datos = d;
            filtrar();
        });
    }
    function filtrar() {
        ...
        const filtrados = datos.filter(row => {
            const txt = !texto || row.Nombre.toLowerCase().includes(texto) || (row.Pais||'').toLowerCase().includes(texto);
            const est = estado === '' || row.Estado == estado;
            return txt && est;
        });
        let html = '<table><thead><tr><th>ID</th><th>Nombre</th><th>Pais</th><th>Estado</th>';
        if (esBib) html += '<th>Acciones</th>';
        html += '</tr></thead><tbody>';
        filtrados.forEach(row => {
            html += `<tr><td>${row.IdEditorial}</td><td>${row.Nombre}</td><td>${row.Pais || '-'}</td><td><span class="status-badge ${row.Estado ? 'status-active' : 'status-inactive'}">${row.Estado ? 'Activo' : 'Inactivo'}</span></td>`;
            if (esBib) html += `<td><button class="btn-sm btn-edit" onclick="window._editEd(${row.IdEditorial})">Editar</button> <button class="btn-sm btn-del" onclick="window._delEd(${row.IdEditorial})">Eliminar</button></td>`;
            html += '</tr>';
        });
        html += '</tbody></table>';
        if (filtrados.length === 0) html = '<p>No se encontraron editoriales</p>';
        else if (filtrados.length < datos.length) html += '<p class="total-reg">Mostrando ' + filtrados.length + ' de ' + datos.length + ' editoriales</p>';
        document.getElementById('tabla').innerHTML = html;
    }
    window._editEd = function(id) {
        const item = datos.find(d => d.IdEditorial === id);
        modal('Editar Editorial', `
            <form id="form"><label>Nombre</label><input name="Nombre" value="${item.Nombre}" required>
            <label>Pais</label><input name="Pais" value="${item.Pais || ''}">
            ...
            api('/editoriales/' + id, { method: 'PUT', body: data }).then(r => {
                if (r.ok) { cargar(); cerrarModal(); mensaje('Guardado'); } else mensaje('Error: ' + r.error);
            });
        });
    };
    window._delEd = function(id) {
        modal('Confirmar', '<p>Eliminar editorial #' + id + '?</p>', function() {
            api('/editoriales/' + id, { method: 'DELETE' }).then(r => {
    ...
    function formulario() {
        modal('Nueva Editorial', `
            ...
            <label>Pais</label><input name="Pais">
            </form>`, function() {
            const data = Object.fromEntries(new FormData(document.getElementById('form')));
            data.Estado = 1;
            api('/editoriales', { method: 'POST', body: data }).then(r => {
                if (r.ok) { cargar(); cerrarModal(); mensaje('Guardado'); } else mensaje('Error: ' + r.error);
            });
        });
    }
