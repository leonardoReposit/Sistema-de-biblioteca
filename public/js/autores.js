(function() {
    ...
    function cargar() {
        ...
        if (esBib) {
            document.getElementById('btnNuevo').innerHTML = '<button class="btn btn-primary" id="btnNuevoAutor">+ Nuevo</button>';
            document.getElementById('btnNuevoAutor').addEventListener('click', function() { formulario(); });
        }
        api('/autores').then(d => {
            datos = d;
            filtrar();
    ...
    function filtrar() {
        ...
        const filtrados = datos.filter(row => {
            const nombre = (row.Nombre + ' ' + row.Apellido).toLowerCase();
            const txt = !texto || nombre.includes(texto) || (row.Nacionalidad||'').toLowerCase().includes(texto);
            const est = estado === '' || row.Estado == estado;
            return txt && est;
        });
        let html = '<table><thead><tr><th>ID</th><th>Nombre</th><th>Nacionalidad</th><th>Nacimiento</th><th>Estado</th>';
        if (esBib) html += '<th>Acciones</th>';
        html += '</tr></thead><tbody>';
        filtrados.forEach(row => {
            html += `<tr><td>${row.IdAutor}</td><td>${row.Nombre} ${row.Apellido}</td><td>${row.Nacionalidad || '-'}</td><td>${row.FechaNacimiento ? row.FechaNacimiento.toString().split('T')[0] : '-'}</td><td><span class="status-badge ${row.Estado ? 'status-active' : 'status-inactive'}">${row.Estado ? 'Activo' : 'Inactivo'}</span></td>`;
            if (esBib) html += `<td><button class="btn-sm btn-edit" onclick="window._editAutor(${row.IdAutor})">Editar</button> <button class="btn-sm btn-del" onclick="window._delAutor(${row.IdAutor})">Eliminar</button></td>`;
            html += '</tr>';
        });
        html += '</tbody></table>';
        if (filtrados.length === 0) html = '<p>No se encontraron autores</p>';
        else if (filtrados.length < datos.length) html += '<p class="total-reg">Mostrando ' + filtrados.length + ' de ' + datos.length + ' autores</p>';
        document.getElementById('tabla').innerHTML = html;
    }
    window._editAutor = function(id) {
        const item = datos.find(d => d.IdAutor === id);
        modal('Editar Autor', `
            <form id="form"><div class="form-grid">
                <div><label>Nombre</label><input name="Nombre" value="${item.Nombre}" required></div>
                <div><label>Apellido</label><input name="Apellido" value="${item.Apellido}" required></div>
                <div><label>Nacionalidad</label><input name="Nacionalidad" value="${item.Nacionalidad || ''}"></div>
                <div><label>Fecha Nac.</label><input name="FechaNacimiento" type="date" value="${item.FechaNacimiento ? item.FechaNacimiento.toString().split('T')[0] : ''}"></div>
            ...
            api('/autores/' + id, { method: 'PUT', body: data }).then(r => {
                if (r.ok) { cargar(); cerrarModal(); mensaje('Guardado'); } else mensaje('Error: ' + r.error);
            });
        });
    };
    window._delAutor = function(id) {
        modal('Confirmar', '<p>Eliminar autor #' + id + '?</p>', function() {
            api('/autores/' + id, { method: 'DELETE' }).then(r => {
    ...
    function formulario() {
        modal('Nuevo Autor', `
            <form id="form"><div class="form-grid">
                ...
                <div><label>Fecha Nac.</label><input name="FechaNacimiento" type="date"></div>
            </div></form>`, function() {
            const data = Object.fromEntries(new FormData(document.getElementById('form')));
            data.Estado = 1;
            api('/autores', { method: 'POST', body: data }).then(r => {
                if (r.ok) { cargar(); cerrarModal(); mensaje('Guardado'); } else mensaje('Error: ' + r.error);
            });
        });
    }
